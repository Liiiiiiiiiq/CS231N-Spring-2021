...
class PositionalEncoding(nn.Module):
    ...
    def __init__(self, embed_dim, dropout=0.1, max_len=5000):
        ...
        t = torch.arange(embed_dim)
        mask = (torch.zeros(max_len, embed_dim) + torch.where(t % 2 == 1, False, True)).bool()
        k = -(torch.zeros(max_len, embed_dim) + torch.where(t % 2 == 1, t - 1, t)) / embed_dim
        k = torch.pow(torch.ones_like(k) * 10000, k) * torch.arange(max_len).reshape((max_len, 1))
        pe[0, :, :] = torch.sin(k) * mask + torch.cos(k) * (~mask)
...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, query, key, value, attn_mask=None):
        ...
        scores = torch.matmul(query, key.transpose(2, 3)) / math.sqrt(d)  # (N, H, S, T)
        if attn_mask is not None:
            attn_mask = attn_mask.bool()
            scores[:, :, ~attn_mask] = float('-inf')
        weights = self.dropout(self.softmax(scores))  # (N, H, S, T)
