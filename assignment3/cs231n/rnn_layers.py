...
def rnn_step_backward(dnext_h, cache):
    ...
    dcache1 = (1 - cache['cache1'] * cache['cache1']) * dnext_h
    db = np.sum(dcache1, axis=0)
    dx = dcache1.dot(cache['Wx'].T)
    dWx = cache['x'].T.dot(dcache1)
    dprev_h = dcache1.dot(cache['Wh'].T)
    dWh = cache['pre_h'].T.dot(dcache1)
...
def rnn_forward(x, h0, Wx, Wh, b):
    ...
    pre_h = h0
    for i in range(T):
        pre_h, cache[f'c{i}'] = rnn_step_forward(x[:, i, :], pre_h, Wx, Wh, b)
        h[:, i, :] = pre_h
    cache['shape'] = (N, T, D, H)
...
def rnn_backward(dh, cache):
    ...
    for i in reversed(range(T)):
        dx[:, i, :], dpre_h, dWx_, dWh_, db_ = rnn_step_backward(dpre_h + dh[:, i, :], cache[f'c{i}'])
        dWx += dWx_
        dWh += dWh_
        db += db_
    dh0 = dpre_h
...
def word_embedding_backward(dout, cache):
    ...
    x, V, D, N, T = cache['x'], cache['V'], cache['D'], dout.shape[0], dout.shape[1]
    dW = np.zeros((V, D))
...
def lstm_step_backward(dnext_h, dnext_c, cache):
    ...
    dnext_c += dnext_h * cache['o'] * (1 - np.power(np.tanh(cache['next_c']), 2))
    dprev_c = dnext_c * cache['f']
    df = dnext_c * cache['prev_c']
    di = dnext_c * cache['g']
    dg = dnext_c * cache['i']
    do = do * (cache['o'] * (1 - cache['o']))
    di = di * (cache['i'] * (1 - cache['i']))
    df = df * (cache['f'] * (1 - cache['f']))
    dg = dg * (1 - np.power(cache['g'], 2))
    dout = np.hstack((di, df, do, dg))
    db = np.sum(dout, axis=0)
    dprev_h = dout.dot(cache['Wh'].T)
    dWh = cache['prev_h'].T.dot(dout)
    dx = dout.dot(cache['Wx'].T)
    dWx = cache['x'].T.dot(dout)
...
def lstm_forward(x, h0, Wx, Wh, b):
    ...
    N, T, H, D = x.shape[0], x.shape[1], h0.shape[1], x.shape[2]
    cache['shape'] = (N, T, H, D)
    h = np.zeros((N, T, H))
    prev_c = np.zeros_like(h0)
    prev_h = h0
    for i in range(T):
        prev_h, prev_c, cache[f'c{i}'] = lstm_step_forward(x[:, i, :], prev_h, prev_c, Wx, Wh, b)
        h[:, i, :] = prev_h
...
def lstm_backward(dh, cache):
    ...
    for i in reversed(range(T)):
        dx[:, i, :], dnext_h, dnext_c, dWx_, dWh_, db_ = lstm_step_backward(dnext_h + dh[:, i, :], dnext_c,
                                                                            cache[f'c{i}'])
        dWx += dWx_
        dWh += dWh_
        db += db_
