...
def sim(z_i, z_j):
    ...
    norm_dot_product = (z_i * z_j).sum() / (z_i.norm() * z_j.norm())
...
def simclr_loss_naive(out_left, out_right, tau):
    ...
    Each row is a z-vector for an augmented sample in the batch. The same row in out_left and
    out_right form a positive pair.
    ...
    for k in range(N):  # loop through each positive pair (k, k+N)
        ...
        for j in range(2 * N):
            if j != k:
                dm1 += (sim(z_k, out[j]) / tau).exp()
            if j != k + N:
                dm2 += (sim(z_k_N, out[j]) / tau).exp()
        total_loss -= ((sim(z_k, z_k_N) / tau).exp().pow(2) / (dm1 * dm2)).log()
...
def sim_positive_pairs(out_left, out_right):
    ...
    pos_pairs = (out_left * out_right).sum(axis=1, keepdim=True) / (
...
def compute_sim_matrix(out):
    ...
    Returns:
    - sim_matrix: 2N x 2N tensor; each element i, j in the matrix is the normalized dot product between out[i]
    and out[j].
    """
    sim_matrix = None
    ...
    sim_matrix = out.mm(out.T) / out.norm(dim=1, keepdim=True).mm(out.norm(dim=1, keepdim=True).T)
...
def simclr_loss_vectorized(out_left, out_right, tau, device='cuda'):
    ...
    exponential = (sim_matrix / tau).to(device).exp()
    ...
    numerator = (sim_pairs / tau).to(device).exp()
